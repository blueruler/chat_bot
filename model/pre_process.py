...
class Corpus:
    #初始化
    def __init__(self, filePath, maxSentenceWordsNum=-1, id2word=None, word2id=None, wordNum=None, tfidf=False, QIDF=None, AIDF=None, testSize=0.2):
        
        self.id2word, self.word2id, self.wordNum = id2word, word2id, wordNum
        #遍历所有内容
        with open(filePath,'r',encoding='utf8') as f:
            txt = self.re_sent(f.readlines()) #一层列表
            data = [i.split('\t') for i in txt] #二层列表
            data = [[jieba.lcut(i[0]), jieba.lcut(i[1])] for i in data] #三层列表
            data = [i for i in data if (len(i[0])<maxSentenceWordsNum and len(i[1])<maxSentenceWordsNum) or maxSentenceWordsNum==-1] #按句子最大词数过滤
            
        self.chatDataWord = data #拷贝
        self.create_dict(data) #生成字典
        #给定word2id,id2word,wordNum 文本text出现未登录词时的处理
        try: 
        ...
        self.QALens(chatDataId) #计算句子（最大）长度（词数）
    ...
    def one_epoch_data_stream(self, batchSize=128, isDataEnhance=False, dataEnhanceRatio=0.2, type='train'):
        ...
        for i in range((len(idList)+batchSize-1)//batchSize):
            ...
            if isDataEnhance:
                ...
                yield QDataId, self.QLens[samples], ADataId, self.ALens[samples]
    #遍历
    def re_sent(self, txt):
        return [filter_sent(qa) for qa in txt]
       
    #数据长度判断
    def QALens(self, data):
